...
int main() {
	...
	cout << endl << "a1 size is after number addition is: " << a1.arraySize() << endl;

	if (a1.check(1.) != -1) // check() returns -1 if number not present
		cout << endl << "1 is present in the array" << endl;

	if (a1.check(5.) != -1)
		cout << endl << "5 is present in the array" << endl;

	cout << endl << "a1 before removal of 2.0: "; a1.output();
	a1.removeNumber(2.);
	cout << endl << "a1 after removal of 2.0: "; a1.output();
	...
	cout << endl << "a3 after stackable assingment: ";
	a3.output();

	a3=a3; // testing protection against self-assingment
	cout << endl << "a3 after self-assignment: ";
	a3.output();
